...
func TestTLSValidation(t *testing.T) {
	...
	t.Run("wrong_CA", func(t *testing.T) {
		...
		s := helpers.NewMCPTestServer(t, "KEYLIME_CA_CERT="+wrongCAPath)
		result := s.CallTool("Get_version_and_health", map[string]any{})
		text := helpers.ExtractText(result)
	...
	t.Run("missing_client_cert", func(t *testing.T) {
		stderr := helpers.StartServerExpectFail(t, "KEYLIME_CLIENT_CERT=/nonexistent/cert.pem")
	...
	t.Run("SNI_mismatch", func(t *testing.T) {
		s := helpers.NewMCPTestServer(t, "KEYLIME_TLS_SERVER_NAME=wrong-hostname.example.com")
		result := s.CallTool("Get_version_and_health", map[string]any{})
		text := helpers.ExtractText(result)
		assert.Contains(t, text, `"reachable":false`)
	})
	...
	t.Run("TLS_disabled", func(t *testing.T) {
		s := helpers.NewMCPTestServer(t, "KEYLIME_TLS_ENABLED=false")
		result := s.CallTool("Get_version_and_health", map[string]any{})
		require.False(t, result.IsError)
	})
}
