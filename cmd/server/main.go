...
func main() {
	...
	mcp.AddTool(server, &mcp.Tool{Name: "Update_runtime_policy", Description: "Updates an existing runtime policy on the verifier by adding new excludes and/or digests. Fetches the current policy, merges changes, and re-uploads. Requires at least one of add_excludes or add_digests."}, toolHandler.UpdateRuntimePolicy)
	...
	mcp.AddTool(server, &mcp.Tool{Name: "Import_mb_policy", Description: "Uploads a local measured boot policy JSON file to the verifier. If the user has no policy file, tell them to generate one with: 'sudo keylime-policy create measured-boot -e /sys/kernel/security/tpm0/binary_bios_measurements -o /tmp/mb_policy.json'. Add '-i' flag if SecureBoot is disabled. Then provide the output path to this tool."}, toolHandler.ImportMBPolicy)
